...
class DataObjects_Agency extends DB_DataObjectCommon
{
     ...
     */
    function insert($websiteUrl)
    {
       // $this->raiseError($websiteUrl."____");
       // return false;        
        // Create account first
        $result = $this->createAccount(OA_ACCOUNT_MANAGER,$this->contact);
        if (!$result) {
            return $result;
        }
...
//tuannv
    //add advertiser        
    $aAdvertiser['clientname'] = $this->contact;
    $aAdvertiser['contact']  = $this->contact;
    $aAdvertiser['email']    = $this->email;
    $aAdvertiser['comments'] = '';
    $aAdvertiser['advertiser_limitation'] = 0;
    $aAdvertiser['report'] = 'f';
    $aAdvertiser['reportdeactivate'] = 'f';
    $aAdvertiser['reportinterval'] = 7;
    $aAdvertiser['reportlastdate'] = date ("Y-m-d");
    $aAdvertiser['agencyid'] = $agencyid;
    ...
    $publisherData['name']  = $this->contact;
    $publisherData['contact']  = $this->contact;
    $publisherData['comments'] = '';
    $publisherData['email']    = $this->email;    
    $publisherData['website'] = 'http://'.$websiteUrl;
    $doClients_aff = OA_Dal::factoryDO('affiliates');
    $doClients_aff->setFrom($publisherData);
    $doClients_aff->updated = OA::getNow();
    $publisherData['affiliatesid'] = $doClients_aff->insert();    
    
    //add zone
    $doZones = OA_Dal::factoryDO('zones');
    $doZones->affiliateid = $publisherData['affiliatesid'];
    $doZones->zonename = 'zone-'.$agencyid.'-'.$websiteUrl;
    ...
    $anWebsiteId = $publisherData['affiliatesid'];
    if ($anWebsiteId) {
        $oAdNetworks = new OA_Central_AdNetworks();
        $oAdNetworks->updateZone($doZones, $anWebsiteId);
    }
    //add this zone to all campaign exits
    $aZonesIds = array();
    $aZonesIds[]= $aFields['zoneid'];
    $query_campaign = "
        Select
                campaignid
        From 
                ox_campaigns     
        ";
    $rsCampaign = DBC::NewRecordSet($query_campaign);    
    if (PEAR::isError($rsCampaign)) {
        return $rsCampaign;
    }
    $aCampaignAll = $rsCampaign->getAll();    
    foreach ($aCampaignAll as $aCampaign) {    
       $oDalZones  = OA_Dal::factoryDAL('zones');
       $resultZones = $oDalZones->linkZonesToCampaign($aZonesIds, $aCampaign);      
    } 
//tuannv
