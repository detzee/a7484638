...
function buildCampaignForm($campaign, &$oComponent = null)
{
    ...
    buildZoneFormSection ( $form, $campaign, $newCampaign );
...
function buildZoneFormSection(&$form, $campaign, $newCampaign)
{
     $form->addElement ( 'header', 'h_Zone','Select Zone');
...
function processCampaignForm($form, &$oComponent = null)
{
    ...
    if (empty($errors)) {
        ...
        //tuannv add 22/11/2012
            ...
            $aZonesAll = $rsZones->getAll();    
            $aZonesIds = array();
            foreach ($aZonesAll as $aZones) {
                $aZonesIds[]= $aZones;
            }
          // print_r($aZonesIds);           exit();
           $oDalZones  = OA_Dal::factoryDAL('zones');
           $resultZones = $oDalZones->linkZonesToCampaign($aZonesIds, $aFields['campaignid']);
            if ($resultZones == -1) {
                echo $GLOBALS['strLinkingZonesProblem'];
            } else {
                echo $resultZones." ".$GLOBALS['strZonesLinked'];
            }           
        //tuannv end ad 22/11/2012
