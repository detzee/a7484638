...
$url_admin=$aConf['webpath']['admin'];
    
    //$out[] = '<script type="text/javascript" src="'.$url_admin.'/plugins/apStatsGraphsUI/js/ofc.js"></script>';
    
    if( !isset( $open_flash_chart_seqno ) )
    {
        $open_flash_chart_seqno = 1;
        $out[] = '<script type="text/javascript" src="http://'.$url_admin.'/plugins/apStatsGraphsUI/js/swfobject.js"></script>';
    }
    else
    {
    ...
    if( $use_swfobject )
    {
		...
		$out[] = 'var so = new SWFObject("http://'.$url_admin.'/plugins/apStatsGraphsUI/open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<embed src="http://'.$url_admin.'/plugins/apStatsGraphsUI/open-flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
